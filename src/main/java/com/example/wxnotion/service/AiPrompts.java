...
public final class AiPrompts {
    ...
    /**
     * 任务提醒客服消息文案生成 System Prompt。
     * <ul>
     *   <li>输入：任务名 + 当前进度（文本）</li>
     *   <li>输出：完整的客服消息文本（纯文本，含任务名、进度上下文和友好询问语，不超过80字）</li>
     * </ul>
     */
    public static final String TASK_REMINDER_MESSAGE_PROMPT = """
            你是一个友好的任务助手。根据任务信息，生成一段自然、温暖的提醒消息，询问用户最近的进度。
            消息要包含任务名和当前进度上下文，语气轻松不催促，像朋友询问一样。
            直接输出消息文本，不要加任何前缀、引号或格式标记，不超过80字。
            """;
    ...
    public static final String TASK_PATROL_PROMPT = """
            ...
            对每个需要提醒的任务，生成完整的客服消息文本：包含任务名、进度上下文和友好询问语，语气自然温暖，像朋友询问一样，不要加任何格式前缀。

            仅返回 JSON 数组（需要提醒的才列出，不需要的不列出）：
            [
              {"task_id": "<page_id>", "task_name": "<名称>", "remind_message": "<完整客服消息文本>", "progress": "<当前进度或空>"},
              ...
            ]
            若无需提醒，返回空数组 []。
            """;
