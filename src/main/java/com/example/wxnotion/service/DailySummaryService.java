...
//            return "用户未配置或未激活";
    ...
    private String processUserSummary(UserConfig user) {
        String apiKey = AesUtil.decrypt(aesKey, user.getEncryptedApiKey());
        String dbId = user.getDatabaseId();

        // 1. 找到今日页面
        String pageId = notionService.findTodayPage(apiKey, dbId);
        if (pageId == null) {
            log.info("用户 {} 今日无页面，跳过总结", user.getOpenId());
            return "今日无页面";
        }
    ...
    private void pushDailyCard(String openId, String aiSummary) {
        ...
        String quote = extractSection(aiSummary, "明日建议");
